/*
 * Standalone temperature reading program
 *
 * - adjust the 'zero' temparature kludge (pcsensor should return proper default to distinguish
 *                                         failure from an exact zero read)
 *
 */
#include <stdio.h>
#include <time.h>
...
int main()
{
  ...
  do {
    ...
    if (!lvr_winusb) {
      ...
      /* here is where the temperature gets read via pcsensor */
  ...
  /* Read fails silently with a 0.0 return, so repeat until not zero or until we have read the
     same zero value 3 times (just in case temp is really dead on zero. */
